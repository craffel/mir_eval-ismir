% Add table captions
% Add conclusion
% Fill in missing segmentation explanations 
...
% Because when numpy computes the median of an even number of numbers, it computes the mean of the two middle values.

\subsection{Segment Frame Clustering}

% Frame sampling - MIREX does two different things, we do one thing consistently
% For Rand index and pairwise classification, MIREX uses a fixed sampling but at different rates.
% In the conditional entropy scores, it uses a fixed number of samples.
% In \mireval{}, we use the same fixed sampling rate for all metrics.

% In MIREX, the range over which frames are sampled depends on both the reference and the estimate intervals.
% In \mireval{}, the sampling is always done with respect to the reference to ensure fair comparison across multiple estimates for the same reference.
...
% In \mireval{}, this is done by adding synthetic intervals when the estimated intervals do not span the reference intervals or otherwise trimming estimated intervals.
...
% When adding intervals, we ensure that the labels do not conflict with existing labels in the estimated annotation.
% This has the effect of changing the normalization constant in the normalized conditional entropy scores.

% When there's only one label, the normalized conditional entropy scores are not well defined.
% In MIREX, they are assigned a score of 1 in this case; in \mireval{} we assign a score of 0.
...
%; and two, we are curious as to how reliability of independently reported results in the literature.
