...
Let $B_R$ ($B_E$) denote the set of unique interval boundaries in the reference (estimated) annotation, and let $W$ denote 
a tolerance window, typically either $0.5$s or $3.0$s.  
A \emph{hit} is defined as a pair $b_e \in B_E$, $b_r \in B_R$ such that $|b_e - b_r|
\leq W$.  

To define detection metrics, we require that no estimated boundary is counted for more than one reference boundary, and vice versa.
This is accomplished by computing a maximum bipartite matching $H_W$ between $B_E$ and $B_R$ (subject to the window constraint $W$) using the Hopcroft-Karp algorithm~\cite{hopcroft1973n}.  
Precision and recall with window $W$ are defined as
\begin{align}
P_W(B_R, B_E) &:= \frac{|H_W|}{|B_E|}\\
R_W(B_R, B_E) &:= \frac{|H_W|}{|B_R|},
\end{align}
...
\begin{align}
\text{R-to-E}(B_R, B_E) &:= \displaystyle\median_{b_r \in B_R} \min_{b_e \in B_E} \left|b_r - b_e\right|\\
\text{E-to-R}(B_R, B_E) &:= \displaystyle\median_{b_e \in B_E} \min_{b_r \in B_R} \left|b_r - b_e\right|.
\end{align}
